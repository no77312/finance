...
PWA 通过 Google 登录。受保护路由会校验用户 session。
...
截图导入解析。可传 OCR 文本，或压缩后的 `imageDataURL`，后端会用 OpenAI 视觉能力解析图片。
...
刷新所有群组的最近一个交易日收盘价。用于 GitHub Actions 或 Render Cron 等定时任务，不由客户端手动调用。
